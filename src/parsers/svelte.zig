...
/// TODO: this entire block (except for referencing `lang_svelte`) is the exact same as the HTML parser's `parse` function.
/// Find a way to abstract the code out, and take in a TS language
fn parse(
    ...
    const QUERY_TAG_NAMES_AND_ATTRS =
        \\(start_tag
        \\  (tag_name) @tagname
        \\  (attribute
    ...
    parser.setLanguage(lang_svelte) catch return &.{};
    ...
    if (parse_res) |ast| {
        ...
        const query_tags_and_attrs = ts.Query.create(lang_svelte, QUERY_TAG_NAMES_AND_ATTRS, &error_offset) catch return &.{};
        defer query_tags_and_attrs.destroy();
        const query_style_blocks = ts.Query.create(lang_svelte, QUERY_STYLE_BLOCKS, &error_offset) catch return &.{};
        defer query_style_blocks.destroy();
        const query_script_blocks = ts.Query.create(lang_svelte, QUERY_SCRIPT_BLOCKS, &error_offset) catch return &.{};
